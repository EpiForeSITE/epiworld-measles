...
## School quarantine model for `r "output"`
...
system2(
    "../../model/./school_quarantine.o",
    c(temp_params_fn, "output"),
...
#| echo: false
cat("```{mermaid}\n")
readLines(file.path("output", "diagram.mmd")) |>
    cat(sep = "\n")
cat("\n```")
```
...
histories <- parallel::mclapply(list.files(
    "output", pattern="total_hist", full.names = TRUE),
   \(fn) {
...
fwrite(
  ...
  file.path(
    "output",
    sprintf("%s_histories_n=%i_vax=%s.csv", "output", n, n_vax)
  )
)

# Reading the 'transition.csv' files`
transitions <- parallel::mclapply(
    list.files("output", pattern="transition", full.names = TRUE),
   \(fn) {
...
fwrite(
  ...
  file.path(
    "output",
    sprintf("%s_transitions_n=%i_vax=%s.csv", "output", n, n_vax)
  )
)
```
...
#| warning: false
#| message: false
repnums <- list.files("output", pattern = "reproduc", full.names = TRUE)
